...
class DataManager:
    ...
    def check_trip_prices(self):
        ...
        for city in self.data:
            ...
            if not lowest_price:  # If there is no data for lowest price
    ...
    def get_current_low_price(self):
        ...
        for entry in self.data:
            city = entry.get("iataCode")
            lowest_price = entry.get("lowestPrice")
            prices_dict[city] = lowest_price
        # print(prices_dict)
        return prices_dict

    def update_lowest_price(self, city, price_of_current_trip):
        self.get_destinations()
        for entry in self.data:
            if entry["iataCode"] == city:
                print(f"Updating lowest price for {city} with price {price_of_current_trip}")
