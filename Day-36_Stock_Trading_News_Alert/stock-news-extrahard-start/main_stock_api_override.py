...
for stock, company in stock_list:
    ...
    message_title = company
    message_body = news_cached.get_latest_news(company)
    message += f"{message_title}\n{message_body}\n"
    if message is not None:  # Checks whether there were news headlines returned or not
        print("Message created!")
    else:
        print(f"No message for {stock} will be sent.")

if message != "":
    get_translation = input("Do you want to apply Japanese translation? (yes/no): ").strip().lower()
    if get_translation == "yes":
        message = translations_deepl.get_translation(message, "JA")
    notifications_email.send_message(message)