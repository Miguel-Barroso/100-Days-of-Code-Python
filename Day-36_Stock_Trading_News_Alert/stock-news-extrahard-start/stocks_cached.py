...
def get_closing_prices_difference(ticker):
    ...
    try:
        ...
        #r = config.cached_session.get(f"{base_url}query?function={api_function}&symbol={ticker}&apikey={api_key}")
        ...
        if r.from_cache:
            print(f"Retrieved {ticker} stock data from cache.")
        else:
            print(f"Retrieved {ticker} stock data fresh from API.")
    except config.requests.exceptions.RequestException as err:
        print(f"❌ Stocks API request failed: {err}")
    ...
    if len(data) == 1:
        print(data)  # Prints the response which usually contains some information
        exit(1)
